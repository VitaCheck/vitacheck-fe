...
let messaging = null;
const shownNotifications = new Set(); // 🔥 중복 방지용
...
try {
  ...
  messaging.onBackgroundMessage(async (payload) => {
    ...
    if (messageId && shownNotifications.has(messageId)) {
      console.log("[SW] Duplicate notification prevented:", messageId);
      return;
    }
    if (messageId) {
      shownNotifications.add(messageId);
      // 메모리 관리: 100개 이상이면 오래된 것 삭제
    ...
    await self.registration.showNotification(title, {
      ...
      data: { url: link, messageId },
      tag: "vitacheck-notification", // 같은 tag는 덮어씀
      renotify: false, // 같은 tag 재표시 안함
...
self.addEventListener("push", (event) => {
  ...
  // 🔥 중복 방지
  if (messageId && shownNotifications.has(messageId)) {
    console.log("[SW] Duplicate standard push prevented:", messageId);
    return;
  }
  if (messageId) {
    shownNotifications.add(messageId);
