...
const P2MDropdownPopup: React.FC<P2MDropdownPopupProps> = ({ 
  ...
  return (
    ...
    >
      <div
        className="bg-white h-[150px] rounded-t-[30px] shadow-lg relative w-full text-center"
        onClick={handlePopupClick}
      >
        <div className="px-[32px] flex flex-col items-start pt-[20px] pb-[27px]">
          <h2 className="text-[22px] font-semibold">목적 선택</h2>
          <div className="w-full flex gap-[12px] mt-[15px] overflow-x-auto hide-scrollbar">
            {selectedItems.map((item, index) => (
              <button
                key={index}
                onClick={() => handleButtonClick(item)}
                className={`flex-shrink-0 h-[38px] text-[16px] font-medium cursor-pointer rounded-full px-[24px] whitespace-nowrap border
