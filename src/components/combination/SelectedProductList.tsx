  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      {selectedItems.map((item, idx) => (
        <div
          key={idx}
          className="relative w-full p-2 rounded-lg border shadow-sm flex flex-col items-center justify-between min-h-[160px] bg-[#EEEEEE]"
        >
          <img
            src={item.imageUrl}
            alt={item.name}
          ...
          />
          <p className="text-xs text-center break-words leading-tight">
            {item.name}
          </p>
          <button
