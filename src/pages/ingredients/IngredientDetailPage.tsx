...
const KAKAO_APP_KEY = import.meta.env.VITE_KAKAO_JS_KEY || '16b75c5d816c501dec98d03ee4340883';
    ...
    <div className="fixed inset-0 z-50">
      ...
      >
        <div className="mx-auto max-w-[440px] rounded-t-3xl bg-white shadow-xl">
          ...
          >
            <span className="inline-flex h-9 w-9 items-center justify-center rounded-full">
              <img src={LINK_ICON} alt="링크" className="h-9 w-9 object-contain" loading="lazy" />
            </span>
...
const IngredientDetailInner = () => {
  ...
  const handleBackClick = () => {
    if (tabHistory.length > 1) {
      ...
      setTabHistory(h);
      setActiveTab(prev);
      window.history.replaceState({ tab: prev, history: h }, '', window.location.href);
    } else {
      setActiveTab('info');
      setTabHistory(['info']);
    }
  ...
  useEffect(() => {
    ...
    window.history.replaceState({ tab: 'info', history: ['info'] }, '', window.location.href);
  ...
  if (!ingredientName)
    return <div className="px-5 py-10">잘못된 접근입니다. 성분명이 없습니다.</div>;
  if (isLoading)
    return (
    ...
    return (
      <div className="px-5 py-10">
        <div className="text-center">
          <p className="mb-2 text-lg text-red-500">성분 정보를 불러올 수 없습니다.</p>
  ...
  if (!data)
    return (
      <div className="px-5 py-10">
        <div className="text-center">
          <p className="mb-2 text-lg text-gray-500">성분 정보를 찾을 수 없습니다.</p>
  ...
  return (
    <div className={`${isMobile ? 'pt-3 pb-5' : 'py-10'}`}>
      ...
      {/* 본문: 탭과 같은 시작점으로 정렬 */}
      <div className={`${WRAP}`}>
        {activeTab === 'info' && <IngredientInfo id={ingredientName!} data={result} />}
