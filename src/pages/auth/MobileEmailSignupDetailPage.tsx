...
const MobileSignupDetailPage = () => {
  ...
  const isSocial = !!base?.socialTempToken || base?.flow === "social";

  useEffect(() => {
    // ✅ 소셜 가입: email/nickname/필수 식별자만 확인
    if (isSocial) {
      if (
        !base?.email ||
        !base?.nickname ||
      ...
      // 프리셋 주입
      if (base?.presetGender) setGender(base.presetGender as any);
    ...
    // ✅ 이메일 가입: 기존 로직 유지 (password 체크)
    if (!base?.email || !base?.password || !base?.nickname) {
      navigate("/signup/email", { replace: true });
      return;
    }
  }, [base, isSocial, navigate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isSocial) {
        ...
        const result = await postSocialSignup(
          body as any,
