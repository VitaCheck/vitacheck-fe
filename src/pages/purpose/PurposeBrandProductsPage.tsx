...
const PurposeBrandProducts = () => {
  ...
  useEffect(() => {
    ...
    const fetchBrandProducts = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get(`http://vita-check.com/api/v1/supplements/brand`, {
          params: { id: brandId },
          headers: { accept: "*/*" },
        });
        ...
        if (response.status === 200) {
          const mappedProducts: Product[] = response.data.supplements.map((item: any) => ({
            id: item.id,
            title: item.name,
            imageUrl: item.imageUrl.startsWith("http") ? item.imageUrl : `/images/${item.imageUrl}`,
          }));
    ...
    <>
      ...
      <div className="sm:hidden">
        <div className="max-w-[430px] mx-auto mt-[50px] pb-[100px]">
          ...
          <div className="mt-[33px] max-w-[430px] w-full justify-items-center
