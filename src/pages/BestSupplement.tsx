...
export default function BestSupplement() {
    ...
    const fetchSupplements = async () => {
      ...
      try {
        const response = await axios.get<ApiResponse>(
          ...
          { params: { ageGroup: apiAgeGroup, gender: apiGender, page: 0, size: 8 } }
        );
  ...
  const renderSkeletonCard = (isMobile: boolean) => (
    <div className="flex flex-col items-center animate-pulse">
      <div className={`${isMobile ? "w-full h-[150px] rounded-xl" : "w-full h-[160px] rounded-[16px]"} bg-gray-200 shadow-lg`} />
    ...
    return products.map((product, index) => {
      ...
      return (
        ...
        >
          ...
          <div className={`${isMobile ? "w-[166px] h-[150px] rounded-xl" : "w-full h-[160px] rounded-[16px]"} bg-white shadow-lg overflow-hidden`}>
            <img
          ...
          <p className={`${isMobile ? "mt-[18px] h-[22px] text-[18px]" : "mt-[16px] text-[22px]"} font-medium text-center`}>
            {product.title}
          </p>
        </div>
      );
    ...
    <>
            ...
            <div className="flex items-center h-[38px] gap-[8px] w-full">
              ...
              <div ref={dropdownRef} className="relative min-w-[91px] h-full cursor-pointer">
                ...
                >
                  ...
                  <svg className="w-[20px] h-[20px] text-black" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
      ...
      <div className="hidden sm:block w-full px-[40px] bg-[#FAFAFA]">
        <div className="max-w-[845px] mx-auto pt-[70px] pb-[80px]">
          <div className="flex flex-col items-start gap-[26px]">
            ...
            <div className="flex items-center h-[34px] gap-[10px] w-full">
              ...
              <div ref={dropdownRef} className="relative min-w-[86px] h-full cursor-pointer">
                ...
                >
                  <span className="pl-[10px] truncate">{selectedGender}</span>
                  <svg className="w-[20px] h-[20px] text-black transition-transform duration-200" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                  </svg>
                </div>
                {isDropdownOpen && (
          ...
          <div className="mt-[40px] grid grid-cols-4 gap-x-[26px] gap-y-[40px]">{renderCards(false)}</div>
        </div>
      </div>
    </>
  );
